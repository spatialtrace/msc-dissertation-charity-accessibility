...
def expand_environment_paths(value):
    ...
    return value
ANNUAL_COLOURS = {2001: "#3b6fb6", 2011: "#d17a22", 2021: "#3a8f5b"}
...
class DescriptiveWorkflow(BaseWorkflow):
    ...
    def make_income_distribution_figure(self) -> pd.DataFrame:
        """Compare charity-level income before and after the capacity transform."""
        income_by_year: dict[int, pd.Series] = {}
        diagnostic_rows = []
        loaded_assignments = getattr(self, "assignments", {})
        for year in YEARS:
            if year in loaded_assignments:
                assignment = loaded_assignments[year]
            else:
                assignment = pd.read_csv(self.config["assignment_paths"][str(year)], low_memory=False)
            if len(assignment) != EXPECTED_CHARITIES[year] or not assignment["charity_number"].is_unique:
                raise AssertionError(f"{year}: Data Spine assignment count/uniqueness changed")
            ...
            if not np.array_equal(
                np.argsort(valid.to_numpy(), kind="stable"),
                np.argsort(transformed.to_numpy(), kind="stable"),
            ):
                raise AssertionError(f"{year}: log1p transformation changed income rank order")
            income_by_year[year] = valid
            diagnostic_rows.append(
                {
                    ...
                    "missing_income_charities": int(income.isna().sum()),
                    "recorded_zero_income_charities": int(valid.eq(0).sum()),
                    "raw_income_skewness": float(valid.skew()),
                    "log1p_income_skewness": float(transformed.skew()),
                    "income_median_2021_gbp": float(valid.median()),
                    "income_p99_2021_gbp": float(valid.quantile(0.99)),
                    "income_max_2021_gbp": float(valid.max()),
        ...
        raw_max = max(float(values.max()) for values in income_by_year.values())
        log_max = max(float(np.log1p(values).max()) for values in income_by_year.values())
        raw_bins = np.linspace(0.0, raw_max, 76)
        log_bins = np.linspace(0.0, log_max, 56)
        fig, axes = plt.subplots(1, 2, figsize=(13.8, 5.7))
        for year in YEARS:
            values = income_by_year[year].to_numpy(dtype=float)
            weights = np.full(values.size, 100.0 / values.size)
            label = f"{year} (n = {values.size:,})"
            axes[0].hist(
                values,
                bins=raw_bins,
                weights=weights,
        ...
        axes[0].set_title("(a) Raw annual income", fontsize=14, fontweight="bold", loc="left")
        axes[1].set_title("(b) log(1 + annual income)", fontsize=14, fontweight="bold", loc="left")
        axes[0].set_xlabel("Annual income (constant 2021 GBP)")
        axes[1].set_xlabel("log(1 + annual income in constant 2021 GBP)")
        axes[0].set_ylabel("Charities per bin (%) — logarithmic scale")
        axes[1].set_ylabel("Charities per bin (%)")
        for axis in axes:
            axis.grid(axis="y", color="#d9d9d9", linewidth=0.7, linestyle="--", alpha=0.8)
        ...
        fig.suptitle(
            "Registered care-related charity income distributions",
            fontsize=17,
            fontweight="bold",
            y=0.98,
        )
        fig.text(
            0.5,
            0.01,
            "Same non-missing charity-level observations in both panels; recorded zeros retained. "
            "The natural-log transformation compresses the extreme upper tail without changing rank order.",
            ha="center",
            fontsize=9,
            color="#555555",
        )
        for text_artist in fig.findobj(Text):
            text_artist.set_fontfamily("Helvetica")
        fig.subplots_adjust(left=0.075, right=0.985, top=0.86, bottom=0.17, wspace=0.25)
        save_figure(fig, self.figure_dir, "charity_income_raw_vs_log_distributions")

        diagnostics = pd.DataFrame(diagnostic_rows)
        write_csv(diagnostics, self.table_dir / "charity_income_transform_diagnostics.csv")
    ...
    def final_qa_and_method(self) -> pd.DataFrame:
        ...
        method = {
            ...
            "income_distribution_figure": "same non-missing charity-level income observations before and after natural-log log1p transformation; recorded zeros retained",
